...
func init() {
	...
	rootCmd.PersistentFlags().BoolVarP(&randomNumber, "random", "r", false, "Append a random 6 digit number")
}

// Execute is responsible for executing the viper command
func Execute() error {
...
func generate(cmd *cobra.Command, args []string) {
	...
	n := 1
	for n <= number {
		name, err := generator.GetName(types, separator, randomNumber)
		if err != nil {
			fmt.Println(err)
			os.Exit(1)
		}
